...
- Node.js v20+
- pnpm (ou npm/yarn)
- O [servidor backend](https://github.com/example/repo/tree/main/backend) deve estar em execução.
...
└── src/
    ...
    │   ├── auth/           # Componentes específicos de autenticação (LoginForm)
    ...
    ├── services/           # Módulos para interagir com APIs externas e armazenamento local
    │   ├── api/            # Configuração do cliente Axios e chamadas à API
    │   └── storage/        # Gerenciamento do armazenamento local (ex: token)
...
- **Login (`/login`):** Usuários existentes podem se autenticar. Em caso de sucesso, um token JWT é recebido e armazenado, e o usuário é redirecionado para o dashboard.
